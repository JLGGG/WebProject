...
<%
		...
		<div id= "button1">
		<p align="left">
			<a href="letter_recieve_list.jsp" style="background:green">받은쪽지</a>
			<a href="letter_send_list.jsp">보낸쪽지</a>
	...
	<%} %>
		<tbody>
			<%
				...
				ArrayList<Letter> list = letterDAO.getRecieveLetterList(userId);
			...
			<tr style="text-align: center; border-bottom: 1px solid #FFE400;">
				<td><%=i + 1%></td>
				<td><a
					href="output_letter.jsp?letterId=<%=list.get(i).getLetterId()%>">
					<%if(list.get(i).isCheckRead() == 0) {%>
						<b><%=list.get(i).getSenderId()%>님이 보낸 쪽지 입니다.</b>
						<%} else{%>
						<%=list.get(i).getSenderId() %>님이 보낸 쪽지입니다.
						<%} %>
				</a></td>
