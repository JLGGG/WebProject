		...
		<tr>
			<td width="8%">
					<input type="button" value="목록" class="btn btn-primary"
						onclick="location.href='review_list_name.jsp'">
			</td>
			<td width="76%">
			<%if(userId.equals(board.getUserId())) {%>
			<td width="8%">
			<input type="button" value="수정하기" class="btn btn-secondary"
						onclick="location.href='edit_review.jsp?postId=<%=postId%>'">
</td>
			<td width="8%">
					<input type="button" value="삭제하기" class="btn btn-secondary"
						onclick="location.href='db_delete_review.jsp?postId=<%=postId%>'">
			</td>
			<%} %>
		</tr>
	</table>
...
<div id="comment">
		...
		<tr style="text-align: center; border-bottom: 1px solid gray;">
			...
			<td> <a href ="db_delete_comment_review.jsp?commentId=<%=commentList.get(i).getCommentId()%>&postId=<%=postId%>">[삭제]</a></td>
			<%}else{ %>
			<td></td>
			<%} %>
		</tr>
		...
		<form name="commentform" method="POST" action="db_comment_review.jsp">
